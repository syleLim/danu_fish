    ...
    def UpdateData(self):
        ...
        for i in range(len(self._PositionList)):
            ...
            if (self._PositionAttributeIdx != -1):
                self._CustomMesh.VBSetVector3Index(i, self._PositionAttributeIdx, self._PositionList[i]) 
            if (self._DiffusesAttributeIdx != -1):
                self._CustomMesh.VBSetVector4Index(i, self._DiffusesAttributeIdx, self._DiffusesList[i])
            if (self._UVAttributeIdx != -1):
                self._CustomMesh.VBSetVector2Index(i, self._UVAttributeIdx, self._UVList[i])
            if (self._IndexAttributeIdx != -1):
                self._CustomMesh.VBSetFloatIndex(i, self._IndexAttributeIdx, float(self._Indices[i]))

            # ※ 지원하는 데이터 형식 : Float, Vector2, Vector3, Vector4
            # VBSet(DataType)      (Buffer Index, Attribute Name, Data)   # Slow (Safe)
            # VBSet(DataType)Index (Buffer Index, Attribute Index, Data)  # Fast
...
class EX_CustomMesh(Actor.Actor):
    ...
    def OnCreate(self, this):
        ...
        self.CustomMesh.CBSetVector3Array("Float3Array", self.Vec3Array)

        self.CustomMesh.CBSetInt("Mode", self._Mode)
        self.CustomMesh.CBSetVector3("Color", Math3d.Vector3(0,1,0))
        
        self.CustomMesh.SetTexture("TextureDiffuse", "$project/Assets/Texture.jpg")

        # ※ 지원하는 데이터 형식 : Int, Float, Vector2, Vector3, Vector4
        # CBSet(DataType)      (Attribute Name, Data)
        # CBSet(DataType)Array (Attribute Name, Data List)
    ...
    def Update(self):
        ...
        self.CustomMesh.CBSetFloat("ColorRate", (BWZeroOne * 0.5 + 0.25))
        ...
        if self._DynamicMeshView == True:
            ...
            self.CustomMesh.CBSetVector3Array("MoveVertex", self.MoveVertexArray)
